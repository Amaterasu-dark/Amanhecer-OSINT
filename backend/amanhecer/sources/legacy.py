...
from ..core.contracts import JsonClient, SourceError
from .brasilapi import CnpjProvider
...
class BrasilApiProvider(RemoteProvider):
    ...
    def collect(self, query: Query) -> Evidence:
        if query.kind == QueryKind.CNPJ:
            return CnpjProvider(self.client).collect(query)
        return super().collect(query)
