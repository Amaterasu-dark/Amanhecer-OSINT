...
from ..core.contracts import AuthorizedJsonClient, JsonResponse, SourceError, SourceHttpError
...
class SerproCpfProvider:
    ...
    def parse(self, response: JsonResponse, query: Query) -> dict[str, str]:
    ...
    def collect(self, query: Query) -> Evidence:
        ...
        try:
            ...
            record = self.parse(response, query)
            ...
            if exc.status_code == 404:
                ...
                return Evidence(self.name, source, "ok", data=data)
            return Evidence(self.name, source, "error", data=data, error=HTTP_MESSAGES.get(exc.status_code, f"Falha HTTP {exc.status_code} na fonte de CPF."))
        except SourceError:
